...
import type { PistaVista } from '../../../shared/mecanicas/pistas';
 ...
 */
export function Cuaderno(): JSX.Element {
  ...
  const bloque = leerBloqueDePistas(vista.estadoDelJuego);
  for (const p of bloque?.misHallazgos ?? []) {
    ...
    >
      <Pantalla>
        ...
        */}
        ...
        {/*
          UNA CARPETA POR CATEGORIA. Aqui habia una sola, rotulada «Los
          objetos» a mano, que pintaba la tercera categoria de CLUEDO.
        */}
        {vista.entidades
          .filter((cat) => cat.cosas.length > 0)
          .map((cat) => (
            <Plegable
              key={cat.categoriaId}
              etiqueta={cat.titulo}
              resumen={cat.cosas.map((o) => o.name).join(' · ')}
            >
              <Marco>
                {cat.cosas.map((o, i) => (
